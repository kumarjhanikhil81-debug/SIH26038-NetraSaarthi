...
export default function ScreeningResultPage() {
  ...
  const rawResult = screeningSession.result || {
    grade: screeningSession.caseData?.predictedGrade ?? 3,
    confidence: screeningSession.caseData?.confidence ?? 95.8,
    lesions: screeningSession.caseData?.lesions ?? CLINICAL_SAMPLE_CASES[3].lesions,
    hotspots: screeningSession.caseData?.gradcamHotspots ?? CLINICAL_SAMPLE_CASES[3].gradcamHotspots,
    lesionMarkers: screeningSession.caseData?.lesionMarkers ?? CLINICAL_SAMPLE_CASES[3].lesionMarkers,
  ...
  // Map numerical grade (0-4) to stateKey for ResultCard & ReferralCard
  const mapGradeToStateKey = (grade) => {
    switch (grade) {
      ...
      default: return 'SEVERE_DR';
    }
  };

  // State switcher to preview all 6 example states
    ...
    switch (key) {
      ...
      default:
        return {
          ...
          confidence: 95.8,
          isLowConfidence: false,
          lesions: CLINICAL_SAMPLE_CASES[3].lesions,
          hotspots: CLINICAL_SAMPLE_CASES[3].gradcamHotspots,
          lesionMarkers: CLINICAL_SAMPLE_CASES[3].lesionMarkers,
          explanation: "Extensive microvascular anomalies detected. Specialist review recommended.",
          fovea: "Mild lipid ring proximity (< 500 μm)",
          quality: 94
        };
    }
  ...
  const stateData = getStateParameters(activeStateKey);
