...
export default function LoginPage() {
  ...
  const { switchRole, language, doctors, setSelectedDoctor } = useApp();
  const [doctorId, setDoctorId] = useState('');
  const [doctorLoginError, setDoctorLoginError] = useState('');
  const [selectedRole, setSelectedRole] = useState('health_worker');

  const handleHealthWorkerLogin = () => {
    switchRole('health_worker');
    navigate('/health-worker');
  };

  const handleDoctorLogin = () => {
    const enteredId = doctorId.trim().toUpperCase();
    const doctor = doctors.find((item) => item.doctorId?.toUpperCase() === enteredId);

    if (!enteredId) {
      setDoctorLoginError('Please enter your Doctor ID.');
      return;
    }

    if (!doctor) {
      setDoctorLoginError('Doctor ID not found. Please register first.');
      return;
    }
      ...
      <div className="relative w-full max-w-4xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          >
            ...
            <button
              onClick={handleHealthWorkerLogin}
            ...
            <div className="mt-6 space-y-3">
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 btn-secondary-large text-base py-3.5 bg-blue-600 hover:bg-blue-700 text-white border-blue-600 hover:text-white"
                >
                  <span>Login</span>
                  <ArrowRight className="w-5 h-5" />
                </button>
                <button
                  ...
                  className="flex-1 py-3.5 rounded-xl border-2 border-blue-200 bg-blue-50 hover:bg-blue-100 text-blue-800 font-bold transition-all"
                >
                  Register
                </button>
              </div>
              <div>
                <input
                  value={doctorId}
                  onChange={(e) => { setDoctorId(e.target.value); setDoctorLoginError(''); }}
                  placeholder="Enter Doctor ID to login"
                  className="w-full px-4 py-3 rounded-xl border border-slate-300 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
                {doctorLoginError && (
                  <p className="mt-2 text-xs font-semibold text-red-600">{doctorLoginError}</p>
                )}
              </div>
            </div>
          </div>
