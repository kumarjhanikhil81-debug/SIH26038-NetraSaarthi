...
export default function PatientRegistrationPage() {
  ...
  const { registerPatient, language } = useApp();
  ...
  const submitPatientToApiPlaceholder = async (formData) => {
    // Simulated network request to POST /api/v1/patients/register
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        // Successful response simulation
        resolve({
          status: 201,
          message: 'Patient registered successfully in anonymized clinical registry',
          data: {
            id: formData.patientId,
            age: Number(formData.age),
            diabetesYears: Number(formData.diabetesDuration),
  ...
  const onSubmit = async (data) => {
    ...
    try {
      // 1. Send data to Backend API Placeholder
      const apiResponse = await submitPatientToApiPlaceholder(data);

      // 2. Synchronize with global state (AppContext) for screening workflow
      const registeredRecord = registerPatient({
        id: data.patientId,
        name: `Patient ${data.patientId}`, // Privacy-preserving label
        abhaId: data.patientId,
        age: Number(data.age),
        gender: 'Not Disclosed (De-identified)',
        village: 'PHC Sector Clinical Worklist',
        diabetesYears: Number(data.diabetesDuration),
        rbs: 220, // Default baseline for screening triage
        hba1c: 8.2,
        notes: data.notes || 'De-identified registration complete.',
      ...
      setApiError('Failed to register patient with backend service. Please check network connectivity and try again.');
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-0.5 font-medium">
            Privacy-preserving diabetic retinal screening intake for rural primary healthcare centers
          </p>
        </div>
      ...
      {/* Privacy & Zero-PII Notice Banner */}
      <div className="p-4 rounded-2xl bg-teal-50/70 border border-teal-200 text-teal-950 flex items-start gap-3 shadow-2xs">
        ...
        <div className="text-xs">
          <div className="font-bold text-teal-900 flex items-center gap-2">
            <span>De-Identified Healthcare Record</span>
          ...
          <p className="text-teal-800/90 mt-0.5 leading-relaxed font-medium">
            NetraSaarthi adheres to healthcare data minimization principles. We do not collect names, phone numbers, or national identification numbers to protect rural patient confidentiality.
          </p>
        </div>
      </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-emerald-800 font-medium mt-0.5">
                Anonymous Record ID: <span className="font-mono font-bold text-slate-900">{apiSuccess.patientId}</span> • Registered in clinical queue.
              </p>
            </div>
          </div>
      ...
      {apiError && (
        <div className="p-4 rounded-2xl bg-rose-50 border border-rose-300 text-rose-950 flex items-start gap-3 shadow-sm">
          ...
          <div className="text-xs">
            <div className="font-bold text-rose-900">Registration Failed</div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <div className="sm:col-span-2">
            ...
            <p className="text-[11px] text-slate-500 mt-1 font-medium">
              Unique de-identified screening code assigned at the primary healthcare center.
            </p>
          </div>
          ...
          <div className="sm:col-span-2">
            ...
            <p className="text-[11px] text-slate-500 mt-1 font-medium">
              Optional non-identifiable medical context to assist the tele-ophthalmology review.
            </p>
          </div>
        ...
        <div className="pt-4 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-slate-100">
          ...
          >
            {isSubmitting ? (
              ...
              <>
                <span>{language === 'hi' ? 'सहेजें और आंख स्कैन करें' : 'Register & Proceed to Eye Scan'}</span>
